...
export default function PreTest() {
  ...
  const generateQuestions = (count: number): Question[] => {
    ...
    for (let i = 0; i < count; i++) {
      let num1 = Math.floor(Math.random() * (15 - 5 + 1)) + 5; 
        let num2 = Math.floor(Math.random() * (15 - 5 + 1)) + 5;
      const operations = ["+", "-", "*", "+", "-", "*"];
      // const operations = [ "-"];
      const operation =
      ...
      if (operation === "+") {
        ...
        num1 = Math.floor(Math.random() * (5 - 1 + 1)) + 1; 
        num2 = Math.floor(Math.random() * (5 - 1 + 1)) + 1;
        correctAnswer = num1 * num2;
      }
  ...
  return (
    <View style={styles.container}>
      <View style={styles.wrapper}>
        <TextBold style={[styles.title, { fontSize: 50 }]}>Pre - Test</TextBold>
