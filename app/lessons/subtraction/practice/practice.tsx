...
export default function Test() {
  const [currentPractice, setCurrentPractice] = useState<"subtraction" | "line">(
    "subtraction"
  );
  ...
  const generateQuestions = (count: number) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      while (options.size < 4) {
        const randomOption = Math.floor(Math.random() * (20 - correctAnswer)) + correctAnswer; // Avoid negatives
  ...
  const generateProblems = () => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      const steps = Math.floor(Math.random() * (startNumber - 1)) + 1; // Ensure steps lead to at least a result of 1
